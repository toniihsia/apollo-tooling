...
class SwiftAPIGenerator extends language_1.SwiftGenerator {
    ...
    classDeclarationForOperation(operation) {
        ...
        this.classDeclaration({
            ...
            if (variables && variables.length > 0) {
                ...
                this.withinBlock(() => {
                    this.printOnNewline((0, printing_1.wrap)(`return [`, (0, printing_1.join)(properties.map(({ name, propertyName }) => `"${name}": ${propertyName}`), ', ') || ':', `]`));
                });
            }
            else {
    ...
    structDeclarationForSelectionSet({ structName, adoptedProtocols = ['GraphQLSelectionSet'], selectionSet }, beforeClosure) {
        ...
        this.structDeclaration({ structName, adoptedProtocols }, () => {
            ...
            this.print((0, printing_1.join)(selectionSet.possibleTypes.map(type => `"${type.name}"`), ', '));
            this.print(']');
            this.printNewlineIfNeeded();
            ...
            const fields = (0, collectAndMergeFields_1.collectAndMergeFields)(typeCase.default, this.context.options.mergeInFieldsFromFragmentSpreads).map(field => this.helpers.propertyFromField(field));
    ...
    initializersForTypeCase(typeCase) {
        ...
        const propertiesForSelectionSet = (selectionSet, namespace) => {
            return (0, collectAndMergeFields_1.collectAndMergeFields)(selectionSet, true)
        ...
        if (variants.length == 0 && typeCase.default.possibleTypes.length == 1) {
            ...
            this.withinBlock(() => {
                this.printOnNewline((0, printing_1.wrap)(`self.init(snapshot: [`, (0, printing_1.join)([
            ...
            for (const variant of remainder ? [remainder, ...variants] : variants) {
                ...
                for (const possibleType of variant.possibleTypes) {
                    ...
                    this.withinBlock(() => {
                        this.printOnNewline((0, printing_1.wrap)(`return ${structName}(snapshot: [`, (0, printing_1.join)([
    ...
    propertyAssignmentForField(field) {
        ...
        const valueExpression = (0, graphql_1.isCompositeType)((0, graphql_1.getNamedType)(type))
            ? this.helpers.mapExpressionForType(type, `$0.snapshot`, (0, language_1.escapeIdentifierIfNeeded)(propertyName))
            ...
            for (const selection of selectionSet.selections) {
                switch (selection.kind) {
                    ...
                    case 'TypeCondition': {
                        ...
                        this.print((0, printing_1.join)([
                            `possibleTypes: [${(0, printing_1.join)(selection.selectionSet.possibleTypes.map(type => `"${type.name}"`), ', ')}]`,
                            'selections: '
                        ], ', '));
                        this.selectionSetInitialization(selection.selectionSet);
                        this.print('),');
    ...
    enumerationDeclaration(type) {
        ...
        this.withinBlock(() => {
            values.forEach(value => {
                ...
                this.printOnNewline(`case ${(0, language_1.escapeIdentifierIfNeeded)(this.helpers.enumCaseName(value.name))} = "${value.value}"`);
            });
        });
        this.printNewline();
    ...
    structDeclarationForInputObjectType(type) {
        ...
        this.structDeclaration({ structName, description, adoptedProtocols }, () => {
            ...
            this.printOnNewline(`public init`);
            this.print('(');
            this.print((0, printing_1.join)(properties.map(({ propertyName, typeName, isOptional }) => (0, printing_1.join)([`${propertyName}: ${typeName}`, isOptional && ' = nil'])), ', '));
            this.print(')');
            this.withinBlock(() => {
                this.printOnNewline((0, printing_1.wrap)(`graphQLMap = [`, (0, printing_1.join)(properties.map(({ name, propertyName }) => `"${name}": ${propertyName}`), ', ') || ':', `]`));
            });
