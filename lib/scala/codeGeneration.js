...
function classDeclarationForOperation(generator, { operationName, operationType, rootType, variables, fields, inlineFragments, fragmentSpreads, fragmentsReferenced, source, sourceWithFragments, operationId }) {
    ...
    (0, language_1.objectDeclaration)(generator, {
        ...
        if (variables && variables.length > 0) {
            ...
            (0, language_1.caseClassDeclaration)(generator, { caseClassName: 'Variables', description: '', params: properties.map(p => {
                    return {
                        name: p.propertyName,
                        type: p.typeName
                    };
...
function caseClassDeclarationForSelectionSet(generator, { caseClassName, parentType, fields, inlineFragments, fragmentSpreads, viewableAs }, beforeClosure, objectClosure) {
    ...
    if (!possibleTypes || possibleTypes.length == 1) {
        properties = fields
            .map(field => (0, naming_1.propertyFromField)(generator.context, field))
            .filter(field => field.propertyName != "__typename");
        (0, language_1.caseClassDeclaration)(generator, { caseClassName, params: properties.map(p => {
                return {
                    name: p.responseName,
                    type: p.typeName,
                };
    ...
    fields.filter(field => (0, graphql_1.isCompositeType)((0, graphql_1.getNamedType)(field.type))).forEach(field => {
        caseClassDeclarationForSelectionSet(generator, {
...
function caseClassDeclarationForInputObjectType(generator, type) {
    ...
    (0, language_1.caseClassDeclaration)(generator, { caseClassName, description, params: properties.map(p => {
            return {
                name: p.propertyName,
                type: p.typeName
            };
